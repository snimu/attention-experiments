"""The below is copied from tysam-code/hlb-gpt except for a few minor changes."""
...
def train(num_steps, attn_type, **kwargs):
    ...
    while current_steps < hyp['opt']['total_train_steps']:
        ...
        if current_steps % 10 == 0 and microbatch_step % current_accumulate_steps == 0 and not current_steps % hyp['opt']['eval_iter'] == 0:
            ...
            train_summary_variables = {'epoch': tokens_seen//len(data['train']), 'crnt_steps': current_steps, 'train_loss': train_loss, 'train_acc': train_acc, 'grad_norm': grad_norm, 'time_secs': total_time_seconds}
